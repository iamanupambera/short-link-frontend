...
import { readTimeSeries, readTopLinks } from './normalize';
import { readNumber } from '@/lib/api/normalize';
import type { DashboardAnalytics, ApiDashboardAnalytics } from '../types/analytics.types';

export async function getDashboardAnalyticsRequest(options?: {
  token?: string | null;
}): Promise<DashboardAnalytics> {
  const response = await apiRequest<ApiDashboardAnalytics>(apiEndpoints.analytics.dashboard, {
    token: options?.token,
  });
  const payload = unwrapData<ApiDashboardAnalytics>(response);
  const record = (isRecord(payload) ? payload : {}) as Partial<ApiDashboardAnalytics> & ApiRecord;
